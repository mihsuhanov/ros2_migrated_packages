...
private:
  ...
  void calculate_path();
  // функция запуска движение робота в заданную целевую позицию
  void move_to_position(double target_x, double target_y, double linear_speed, double angular_speed);
  // Обработка данных из /odom и обновление текущих координат
...
private:
  ...
  ros::Subscriber odom_subscriber_ = nh_.subscribe<nav_msgs::Odometry>("/odom", 1, &Planner::odomCallback, this);
